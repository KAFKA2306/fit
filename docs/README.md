...
│   └── smoothing.md    # スムージング
└── note/             # 読み物
    ├── project_concept.md  # コンセプト
    ├── rbf_intuition.md    # RBFって何？（初心者向け）
    ├── obb_intuition.md    # 自動仕分けの魔法（OBB）
    ├── weights_intuition.md # 透明な糸の話（ウェイト）
    ├── shapekeys_intuition.md # 服が痩せる仕組み（シェイプキー）
    ├── smoothing_intuition.md # ギザギザを消す技術（スムージング）
    ├── consolidation_intuition.md # 靴がくしゃっとならない理由（ウェイト統合）
    └── batch_intuition.md  # 寝ている間に終わる（バッチ処理）
```
